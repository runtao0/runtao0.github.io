...
/***/ function(module, exports, __webpack_require__) {
	...
	$(function () {
	  ...
	  function onDocumentMouseDown(event) {
	    ...
	    scene.mouse.set(event.clientX / window.innerWidth * 2 - 1, -(event.clientY / window.innerHeight) * 2 + 1);
	    scene.raycaster.setFromCamera(scene.mouse, scene.camera);
	    var blockIntersects = scene.raycaster.intersectObjects(scene.grid.cells);
	    var planeIntersects = scene.raycaster.intersectObjects(objects);
	    ...
	    if (intersects.length > 0) {
	      ...
	      if (isShiftDown) {
	        ...
	        console.log(lifeTracker[gridX + ',' + gridY]);
	      }
	      scene.render();
	    }
	  }
